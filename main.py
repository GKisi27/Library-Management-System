    ...
    def get_issues_data(self):
        ...
        if result:
            ...
            for row,item in enumerate(result):
                ...
                i_due_date.setTextAlignment(0x0004)
                i_return_date = QTableWidgetItem(str(item['return_date']))
                i_return_date.setTextAlignment(0x0004)
                i_fine = QTableWidgetItem(str(item['fine']))
                i_fine.setTextAlignment(0x0004)
...
if __name__ == "__main__":
    ...
    style_path = os.path.join(dir_path, 'style.qss')
    print(style_path)
